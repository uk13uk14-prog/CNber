/**
 * 与后端 canDispatchByDeposit / 支付展示对齐（不绕过定金确认规则）
 */
...
/** 订单主状态展示：deposit_paid 但未 admin 确认时不显示「已付订金」 */
...
export function depositRemainingSummaryLabel(order) {
  if (!order) return '—'
  const dep = depositDisplayLabel(order)
  const rem = order.remainingPaid ? '尾款已付' : '尾款未付'
  return `${dep}；${rem}`
}
