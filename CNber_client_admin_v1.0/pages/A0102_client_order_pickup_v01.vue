    ...
    <view class="form-card">
      ...
      <view class="row-3">
        <picker :range="airportDisplayList" @change="onAirportChange">
          <view class="picker airport-picker">{{ selectedAirport || '机场' }}</view>
        </picker>
      ...
      <view class="row-2">
        <picker mode="date" :start="minDate" @change="(e) => form.pickupDate = e.detail.value">
          <view class="picker pickup-date">{{ form.pickupDate || '接机日期' }}</view>
        </picker>
        <picker mode="time" @change="(e) => form.pickupTime = e.detail.value">
          <view class="picker pickup-time">{{ form.pickupTime || '接机时间' }}</view>
        </picker>
      </view>
      ...
      <view class="tip">
        🚗 提示：接机免费等待时间为90分钟。因高峰期出关和行李问题，建议您在选着时间往后（如：延后60分钟），以避免落地后即开始计算额外等待费用。
      </view>
      ...
      <view class="baby-seat">
        ...
        <radio-group @change="handleBabySeatChange">
          <label><radio value="none" checked /> 无</label>
          <label><radio value="0-2" /> 0-2岁</label>
          <label><radio value="2-6" /> 2-6岁</label>
        </radio-group>
...
.pickup-time { width: 90%; }
.dropoff-address { width: 100%; }
.vehicle-picker { width: 101%; }
