#!/usr/bin/env python3
"""Deploy client coupon V1 (backend + admin) to M1."""
import os
import sys
import time
...
BACKEND_FILES = [
    "models/Order.js",
    "utils/couponOrder.js",
    "controllers/orderPaymentController.js",
    "scripts/smoke_order_coupon.js",
]

ADMIN_FILES = [
    "src/utils/currencyDisplay.js",
...
def main():
    ...
    sftp.close()
    run(c, f"cd {REMOTE_BACKEND} && node scripts/smoke_order_coupon.js")
    run(c, f"cd {REMOTE_BACKEND} && pm2 restart cnber-backend --update-env")
    time.sleep(3)
    run(
        c,
        "curl -s 'http://127.0.0.1:3100/api/public/coupons/validate?code=NEW100&serviceType=point&vehicleClass=standard_5&amountCny=900' | head -c 300",
    )
    c.close()
    print("\nDONE")
