#!/usr/bin/env python3
"""Deploy driver settlement V1 to M1."""
import os
import sys
import time
...
BACKEND_FILES = [
    ...
    "utils/driverSettlementPeriod.js",
    "controllers/driverSettlementController.js",
    "routes/admin.js",
    "scripts/smoke_driver_settlement.js",
]
...
def main():
    ...
    sftp.close()
    run(c, f"cd {REMOTE_BACKEND} && node scripts/smoke_driver_settlement.js")
    run(c, f"cd {REMOTE_BACKEND} && pm2 restart cnber-backend --update-env")
    time.sleep(3)
    run(c, f"ls {REMOTE_ADMIN}/assets/DriverSettlementsView*.js 2>/dev/null")
    c.close()
    print("\nDONE")
