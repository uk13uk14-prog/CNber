#!/usr/bin/env python3
"""Deploy staff RBAC to M1: admin dist + backend staff files."""
import os
import sys
...
def main():
    ...
    run(c, f"grep -R '员工管理' {REMOTE_ADMIN}/assets/MainLayout*.js | head -1")
    run(c, "curl -s -o /dev/null -w 'admin HTTP %{http_code}\\n' http://127.0.0.1:3100/admin/")
    run(c, f"cd {REMOTE_BACKEND} && node scripts/smoke_staff.js")
    c.close()
    print("\nDONE")
